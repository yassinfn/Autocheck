      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-200">
            <th className="text-left py-2 pr-4 text-xs font-semibold text-slate-500 uppercase tracking-wide">{L.poste}</th>
            <th className="text-left py-2 pr-4 text-xs font-semibold text-slate-500 uppercase tracking-wide hidden sm:table-cell">{L.detail_col}</th>
            <th className="text-right py-2 text-xs font-semibold text-slate-500 uppercase tracking-wide">{L.fourchette}</th>
          </tr>
        </thead>
        <tbody>
          {items.map((item, i) => (
...
export default function DepensesBlock({ depenses, symbole, labels }: DepensesBlockProps) {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div className="p-5 space-y-7">
        <div>
          <h3 className="text-sm font-semibold text-slate-700 mb-4">{L.depenses_mecaniques}</h3>

          <div className="mb-5">
            <SectionLabel>{L.obligatoire_achat}</SectionLabel>
            <ItemTable
              items={obligatoires}
              symbole={symbole}
              footer={{ label: L.total_certain, min: totalMin, max: totalMax }}
              L={L}
            />
          </div>
          ...
          {eventuelles.length > 0 && (
            <div>
              <SectionLabel>{L.selon_diagnostic}</SectionLabel>
              <p className="text-xs text-slate-400 mb-3">{L.montants_non_inclus}</p>
              <ItemTable items={eventuelles} symbole={symbole} L={L} />
            </div>
          )}
        </div>
        ...
        {fraisAchat.length > 0 && (
          <div className="pt-5 border-t border-slate-200">
            <h3 className="text-sm font-semibold text-slate-700 mb-4">{L.frais_achat}</h3>
            <SectionLabel>{L.carte_grise}</SectionLabel>
            <ItemTable items={fraisAchat} symbole={symbole} L={L} />
          </div>
        )}
      </div>
    </div>
